...
def lora_forward(self, input: torch.Tensor) -> torch.Tensor:
    ...
    if hasattr(self, 'lora_A') and hasattr(self, 'lora_B'):
        lora_out = F.linear(F.linear(input, self.lora_A), self.lora_B) * self.scaling
...
def train_batch(config: PretrainConfig, train_state: TrainState, batch: Any, global_batch_size: int, rank: int, world_size: int):
    ...
    if len(metrics):
        ...
        if rank == 0:
            metric_values = metric_values.cuda().numpy()
...
def evaluate(config: PretrainConfig, train_state: TrainState, eval_loader: torch.utils.data.DataLoader, eval_metadata: PuzzleDatasetMetadata, rank: int, world_size: int):
    with torch.inference_mode():
            ...
            for collection in (batch, preds):
                for k, v in collection.items():
                    if k in config.eval_save_outputs:
                        ...
                        all_preds[k].append(v.cuda())  # Move to CPU for saving GPU memory
        ...
        if metric_values is not None:
            ...
            if rank == 0:
                reduced_metrics = metric_values.cuda().numpy()
