    ...
    "cells": [
        ...
        # ---- Cell 2: Patch flash_attn ----
        {
            ...
            "source": [
                ...
                "\n",
                "# Patch finetune_lora.py to use CUDA instead of CPU\n",
                "with open('finetune_lora.py', 'r') as f:\n",
                "    content = f.read()\n",
                "content = content.replace('\"cpu\"', '\"cuda\"').replace('.cpu()', '.cuda()').replace('.cuda().numpy()', '.cpu().numpy()')\n",
                "with open('finetune_lora.py', 'w') as f:\n",
                "    f.write(content)\n",
                "print('Patched finetune_lora.py to use CUDA')\n"
            ]
        },
